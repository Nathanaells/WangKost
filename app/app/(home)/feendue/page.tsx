...
export default function FeenDues() {
  ...
  const [allPayments, setAllPayments] = useState<IPayment[]>([]); // For summary calculation
  ...
  // Fetch ALL data once for summary cards (no pagination)
  useEffect(() => {
    async function fetchAllData() {
      try {
        // Fetch without pagination parameters to get all data
        const result = await fetchTransactionsData(1, 999999); // Very large limit to get all

        if (result.success) {
          setAllPayments(result.payments || []);
        }
      } catch (error) {
        console.error("Failed to fetch all transactions for summary:", error);
      }
    }
  ...
  // Calculate totals from ALL payments (not paginated)
