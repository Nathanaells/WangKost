...
export default function AddAdditionalButton({
  rentId,
  allAdditionals,
  currentAdditionals,
  slug,
  roomId,
}: AddAdditionalButtonProps) {
  ...
  const availableAdditionals = allAvailableAdditionals.filter(
    (a) => !currentAdditionalIds.includes(a._id)
  );
  ...
  const handleCreateCustom = async () => {
    ...
    try {
      const result = await createCustomAdditional(
        customAdditional.name,
        customAdditional.price
      );
      ...
      if (additionalsResult.success && additionalsResult.data) {
        const newAdditional = additionalsResult.data.find(
          (a: IAdditional) =>
            a.name === customAdditional.name &&
            a.price === customAdditional.price
        );
      ...
      const errorMsg =
        error instanceof Error ? error.message : "Failed to create additional";
      showError(errorMsg);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      for (const additionalId of selectedAdditionalIds) {
        try {
          const result = await addAdditionalToRent(
            rentId,
            additionalId,
            slug,
            roomId
          ...
          if (!result.success) {
            failedCount++;
            const additional = allAvailableAdditionals.find(
              (a) => a._id === additionalId
            );
            errors.push(
              `${additional?.name || "Service"}: ${result.message || "Failed"}`
            );
          } else {
            successCount++;
          }
      ...
      if (successCount > 0) {
        showSuccess(
          `${successCount} additional service(s) added successfully!`
        );
      }

      if (failedCount > 0) {
        showError(
          `Failed to add ${failedCount} service(s): ${errors.join(", ")}`
        );
      }
      ...
      showError(
        (error as Error).message || "Failed to add additional services"
      );
    } finally {
      setSubmitting(false);
    }
                          ...
                          <div className="absolute z-10 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg max-h-60 overflow-y-auto">
                            {availableAdditionals.length === 0 ? (
                              ...
                              availableAdditionals.map((additional) => {
                                ...
                                const isDisabled =
                                  currentAdditionalIds.includes(additional._id);
                                ...
                                return (
                                  ...
                                  >
                                    ...
                                    <span className="text-sm">
                                      Rp{" "}
                                      {additional.price.toLocaleString("id-ID")}
                                    </span>
                                  </div>
                                );
                              })
