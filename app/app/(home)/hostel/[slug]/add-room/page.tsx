...
async function createRoom(slug: string, fixedCost: number) {
  try {
    ...
    return {
      success: false,
      message: error as string | "Failed to create room",
    };
  }
}
...
export default function AddRoomPage() {
  ...
  useEffect(() => {
    async function initializeRooms() {
      ...
      setRooms([
        { id: startIndex, roomName: `Room ${startIndex}`, fixedCost: 0 },
      ]);
      setLoading(false);
    }
    initializeRooms();
  ...
  const handleAddRoom = () => {
    ...
    setRooms([
      ...rooms,
      { id: newId, roomName: `Room ${newId}`, fixedCost: 0 },
    ]);
  };
  ...
  const handleFixedCostChange = (id: number, value: number) => {
    setRooms(
      rooms.map((room) =>
        room.id === id ? { ...room, fixedCost: value } : room
      )
    );
  };
